...
%						  	data import (see APPLESEED_setup())}
...
% About:
%
% 		APPLESEED (Puglia, Slobin, & Williams, 2021) preprocesses task-based or resting-state 
...
%			6, 23073. 
%		Puglia, M.H., Slobin, J.S., Williams, C.L., 2021. The Automated Preprocessing 
%			Pipe-Line for the Estimation of Scale-wise Entropy from EEG Data (APPLESEED): 
%			Development and validation for use in pediatric populations. bioRxiv.
...
%		directory in MATLAB. The example code in APPLESEED_batch will then run APPLESEED_setup()  
%		and APPLESEED() from the MATLAB command line on the example dataset which includes:  
...
function APPLESEED(filenamebase, parentdir, varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	%Establish final file name 	
	FinalFileName=[filenamebase, '_srate', num2str(p.resamp), '_filt', num2str(p.hp), '-', num2str(p.lp)];
	if isfield(p,'binfilename')
		FinalFileName=[FinalFileName, '_', strrep(p.binfilename, '.txt', ''), '_be0_', num2str(p.eplen)];
	end
	if strcmp(p.runica,'on') == 1  
		FinalFileName=[FinalFileName,'_arxt','_ica','_cleaned_', p.icarejmethd];
	end
	FinalFileName=[FinalFileName,'_arsd'];
	if strcmp(p.chaninterp, 'on') == 1
		FinalFileName=[FinalFileName,'_chaninterp'];
	end
	FinalFileName=[FinalFileName,'_avgref', '_trialsel', p.trselmethd, num2str(p.trselcnt)];
	...
	if ~isfield(p,'binfile') %rest
		...
		if contains([EEG.urevent.code], 'Stimulus')
			p.taskwarn=length(strfind([EEG.urevent.code],'Stimulus'));
		end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	if isfield(p, 'resampi')
		...
		[ALLEEG,EEG,CURRENTSET] = pop_newset(ALLEEG, EEG, 0,'setname', CurrentFileName,'gui','off');
	...
	if isfield(p,'binfilename') ~= 0 %task-based analysis		
		...
		FileNameAppend=['_', strrep(p.binfilename, '.txt', ''), '_be0_', num2str(p.eplen)];
		CurrentFileName=[CurrentFileName,FileNameAppend];
		...
		a=hist([EEG.event.bini],1:length(find(unique([EEG.event.bini])>0)));
		

		%Write to logfile
		if isempty(find(unique([EEG.event.bini])>0)) || sum(a(1:length(find(unique([EEG.event.bini])>0)))>=p.trselcnt)~=length(find(unique([EEG.event.bini])>0))		
		...
		[EEG.data,tmptime,indices,epochevent]= epoch(EEG.data, [EEG.event.latency], [0 p.eplen/1000]*EEG.srate);
	...
	if strcmp(p.runica,'on') == 1 	
		...
		%%%%%%%%%%%%%%%%%%%%%%%%%
		FileNameAppend=['_cleaned_', p.icarejmethd];
		CurrentFileName=[CurrentFileName,FileNameAppend];
	...
	CurrentFileName=[CurrentFileName,FileNameAppend];
		...
		%Artifact detection SD: (Based on "Neural correlates of infantsÕ sensitivity to vocal expressions of peers")	
		...
		%Check if enough trials to continue
		if length(find(EEG.reject.rejmanual == 1)) == EEG.trials
			fprintf(logfile,'     FAILURE: 100%% of epochs rejected. Insufficient epochs to continue.\n\n%s\n\nAnalysis terminated\n\n%s',repmat('-',1,200), char(datetime),repmat('-',1,200));
			fclose(logfile);
			fprintf('%s\n%s%s\n%s\n',repmat('%',1,100),'% APPLESEED completed. See analysis log file: ',logfilename,repmat('%',1,100));
			close all;
			return;
	...
	if exist(fullfile(outdir, [CurrentFileName, '.set']), 'file')== 0 %if file does not exist
		...
		if p.reref=='Average'
			...
			for r=1:size(p.reref,2)
				rerefs=horzcat(rerefs, find(strcmpi({EEG.chanlocs.labels},p.reref{r})));
			end			
	...
	%Select trials: GFP 			
		...
		for j = 1:size(EEG.data, 3)
			...
			P1=sum(x.^2);
			P2=(sum(x).^2)/size(x,2);
			TotSS=horzcat(TotSS,P1-P2);
		end %for trials


		TrialsToKeep=[];
		for b = 1:length(find(unique([EEG.event.bini])>0))
			TrialSelect{b}.Trials=find([EEG.event.bini]==b);
			if length(TrialSelect{b}.Trials)>p.trselcnt
				[~,TrialSelect{b}.SortedIndices]= sort(abs(TotSS(TrialSelect{b}.Trials)-median(TotSS)));
				TrialSelect{b}.SortedTrials=TrialSelect{b}.Trials(TrialSelect{b}.SortedIndices);
				TrialSelect{b}.KeepTrials=sort(TrialSelect{b}.SortedTrials(1:p.trselcnt));
			else
				TrialSelect{b}.KeepTrials=find([EEG.event.bini]==b);
			end
			TrialsToKeep=horzcat(TrialsToKeep,TrialSelect{b}.KeepTrials);
		end %for bins
		...
		for b = 1:length(find(unique([EEG.event.bini])>0))
			...
			for Chan = 1:size(EEG.data,1)
				...
				for Trial = 1:size(EEG.data,3)
					Tmp=horzcat(Tmp,EEG.data(Chan,:,Trial)');
				end %for trial
				Raw{Chan}=Tmp;
				Mean{Chan}=mean(Raw{Chan},2);
				Res{Chan}=Raw{Chan}-Mean{Chan};
			end %for chan		
			...
			for Chan = 1:EEG.nbchan
				...
				EntropyOut=horzcat(EntropyOut,SE');
			end %for channel
